...
export class RenderService {
  ...
  private drawNode(ctx: CanvasRenderingContext2D, node: Node, viewState: ViewState): void {
    ...
    const headerHeight = NODE_HEADER_HEIGHT; // Not scaled, it's a conceptual height

    const isSelected = this.selectionManager.isSelected(node.id);
    ctx.fillStyle = this.themeColors.nodeBackground;
    ctx.strokeStyle = isSelected ? this.themeColors.selectionHighlight : this.themeColors.nodeBorder;
    ...
    // Icon
    if (node.icon) {
        ctx.fillStyle = this.themeColors.nodeIconBackground;
        const iconX = x + 8; const iconY = y + 8; const iconSize = 24;
        ...
        // This assumes iconService.getIconHTML or similar is used by NodeEditorController to set node.icon appropriately,
        // or that node.icon is a direct character or simple identifier.
        // For simplicity, let's assume node.icon is a short text/character.
  ...
  private drawPort(ctx: CanvasRenderingContext2D, port: NodePort, canvasX: number, canvasY: number, viewState: ViewState, isHighlighted: boolean = false): void {
    ...
    const borderColor = port.isDynamic
        ...
        : (isHighlighted ? this.themeColors.portHighlightBorder : this.themeColors.nodePortBorder);
    const fillColor = port.isDynamic
        ? this.themeColors.nodePortDynamicBackground
        : (isHighlighted ? this.themeColors.portHighlightFill : this.themeColors.nodePortBackground);
  ...
  private drawStickyNote(ctx: CanvasRenderingContext2D, note: StickyNote, viewState: ViewState): void {
    const x=note.position.x; const y=note.position.y;
