...
export class RenderService {
  ...
  constructor(
    ...
    this.interactionManager.on(
      "compatiblePortHoverChanged",
      this.handleCompatiblePortHoverChanged
    );
    this.interactionManager.on(
      "portHoverChanged",
      this.handlePortHoverIdleChanged
    );
  ...
  private handleViewChange = (newState: ViewState, oldState?: ViewState) => {
    const needsAnimation =
      newState.preferences.connectionAppearance.animateFlow;
    const wasAnimating =
      oldState?.preferences.connectionAppearance.animateFlow ?? false;
  ...
  private startAnimationLoop = () => {
    ...
    const animate = (currentTime: number) => {
      ...
      const hasAnimatedConnections = this.connectionManager
        .getConnections()
        .some((conn) => conn.style?.animated);
  ...
  private loadThemeColors(): void {
    const computedStyles = getComputedStyle(
      this.canvasEngine.getCanvasElement()
    );
    this.themeColors = {
      nodeBackground: computedStyles
        .getPropertyValue("--node-background-color")
        .trim(),
      nodeBorder: computedStyles.getPropertyValue("--node-border-color").trim(),
      nodeHeaderBackground: computedStyles
        .getPropertyValue("--node-header-background-color")
        .trim(),
      nodeIconBackground: computedStyles
        .getPropertyValue("--node-icon-background-color")
        .trim(),
      nodeIconText: computedStyles
        .getPropertyValue("--node-icon-text-color")
        .trim(),
      nodeTitleText: computedStyles
        .getPropertyValue("--node-title-text-color")
        .trim(),
      nodeIdText: computedStyles
        .getPropertyValue("--node-id-text-color")
        .trim(),
      nodePortBackground: computedStyles
        .getPropertyValue("--node-port-background-color")
        .trim(),
      nodePortBorder: computedStyles
        .getPropertyValue("--node-port-border-color")
        .trim(),
      nodePortText: computedStyles
        .getPropertyValue("--node-port-text-color")
        .trim(),
      nodePortDynamicBackground: computedStyles
        .getPropertyValue("--node-port-dynamic-background-color")
        .trim(),
      nodePortDynamicBorder: computedStyles
        .getPropertyValue("--node-port-dynamic-border-color")
        .trim(),

      connectionDefault: computedStyles
        .getPropertyValue("--connection-default-color")
        .trim(),
      connectionPending: computedStyles
        .getPropertyValue("--connection-pending-color")
        .trim(),
      connectionPendingCompatible: computedStyles
        .getPropertyValue("--connection-pending-compatible-color")
        .trim(),
      connectionReconnecting: computedStyles
        .getPropertyValue("--connection-reconnecting-color")
        .trim(),
      connectionGhosted: computedStyles
        .getPropertyValue("--connection-ghosted-color")
        .trim(),

      selectionHighlight: computedStyles
        .getPropertyValue("--selection-highlight-color")
        .trim(),
      selectionBoxFill: computedStyles
        .getPropertyValue("--selection-highlight-color-transparent")
        .trim(),
      portHighlightBorder: computedStyles
        .getPropertyValue("--port-highlight-border-color")
        .trim(),
      portHighlightFill: computedStyles
        .getPropertyValue("--port-highlight-fill-color")
        .trim(),

      statusSuccess: computedStyles
        .getPropertyValue("--status-success-color")
        .trim(),
      statusError: computedStyles
        .getPropertyValue("--status-error-color")
        .trim(),
      statusRunning: computedStyles
        .getPropertyValue("--status-running-color")
        .trim(),
      statusWarning: computedStyles
        .getPropertyValue("--status-warning-color")
        .trim(),
      statusUnsaved: computedStyles
        .getPropertyValue("--status-unsaved-color")
        .trim(),

      stickyNoteDefaultBackground: computedStyles
        .getPropertyValue("--sticky-note-default-background-color")
        .trim(),
      stickyNoteDefaultText: computedStyles
        .getPropertyValue("--sticky-note-default-text-color")
        .trim(),
      stickyNoteBorder: computedStyles
        .getPropertyValue("--sticky-note-border-color")
        .trim(),
    };
  }
  ...
  private renderGroups(): void {
    ...
    groups.forEach((group) =>
      this.drawGroup(this.ctx!, group, this.currentViewState!)
    );
  }
  ...
  private renderStickyNotes(): void {
    ...
    notes.forEach((note) =>
      this.drawStickyNote(this.ctx!, note, this.currentViewState!)
    );
  }
  ...
  private renderBoxSelect(): void {
    ...
    if (boxRect) {
      this.ctx.fillStyle =
        this.themeColors.selectionBoxFill || "rgba(99, 102, 241, 0.1)";
  ...
  private drawNode(
    ...
    if (node.isComposite) {
      this.drawCompositeNode(ctx, node, viewState);
      // Não execute o resto do desenho de nó padrão para nós compostos
      return;
    }
    ...
    if (node.icon) {
      ...
      ctx.textBaseline = "middle";
      ctx.fillText(
        node.icon.substring(0, 2).toUpperCase(),
        iconX + iconSize / 2,
        iconY + iconSize / 2 + 1
      );
    }
  ...
  private drawCompositeNode(
    ...
    const borderColor = node.color || "#94a3b8";
  ...
  private drawPort(
    ...
    const nodeCustomColor =
      parentNode?.color || this.themeColors.nodePortBorder;

    const borderColor = port.isDynamic
      ? this.themeColors.nodePortDynamicBorder
      : isHighlighted
      ? this.themeColors.portHighlightBorder
      : nodeCustomColor;
  ...
  private drawStickyNote(
    ...
    ctx.fillStyle =
      ...
      this.themeColors.stickyNoteDefaultBackground;
    ...
    ctx.fillStyle =
      note.style.textColor || this.themeColors.stickyNoteDefaultText;
  ...
  private renderPendingOrReconnectingConnection(): void {
    ...
    if (pendingConn) {
      ...
      const cp1x =
        pendingConn.fromPosition.x +
        (50 / this.currentViewState.scale) * curveFactor;
      const cp1y = pendingConn.fromPosition.y;
      const cp2x =
        endPoint.x - (50 / this.currentViewState.scale) * curveFactor;
      ...
      if (pendingConn.reconnectingInfo)
        strokeColor = this.themeColors.connectionReconnecting;
      if (pendingConn.compatibleTargetPortId)
        strokeColor = this.themeColors.connectionPendingCompatible;

      this.ctx.strokeStyle = strokeColor;
      this.ctx.lineWidth = 2 / this.currentViewState.scale;
      this.ctx.setLineDash([
        4 / this.currentViewState.scale,
        2 / this.currentViewState.scale,
      ]);
      this.ctx.stroke();
      this.ctx.setLineDash([]);
    }
  ...
  private renderConnections(): void {
    ...
    const pendingReconInfo =
      this.interactionManager.getPendingConnection()?.reconnectingInfo;

    connections.forEach((conn) => {
      if (
        pendingReconInfo &&
        pendingReconInfo.originalConnection.id === conn.id
      ) {
  ...
  private drawConnection(
    ...
    const p3 = this.interactionManager.getPortAbsolutePosition(
      targetNode,
      targetPort,
      viewState
    );
  ...
  private setupConnectionStyle(
    ...
    const isAnimated =
      viewState.preferences.connectionAppearance.animateFlow &&
      conn.style?.animated;
